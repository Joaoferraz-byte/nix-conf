...
{ ... }: {
  flake.nixosModules.dellLatitude5410Hardware = { config, lib, pkgs, modulesPath, ... }: {
    imports = [
      (modulesPath + "/installer/scan/not-detected.nix")
    ];

    # ── Módulos do kernel (initrd) ────────────────────────────────────────
    # Dell Latitude 5410: xhci_pci, ahci, nvme, usb_storage, usbhid, sd_mod

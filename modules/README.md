...
The selected plugin set is vendored in and pinned by `shell-conf`/`noctalia-conf`: `cat`, `timer`, `screen_toolkit`, `gamer_mode`, the FreeSM-adapted `prismlauncher_instances` provider and `bitwarden`. Screen Toolkit is the single owner of capture and recording, consuming the system-provided `gpu-screen-recorder`, `wl-screenrec` and Wayland/OCR/annotation tools from `features/niri.nix`. Plugin source is immutable; plugin settings and runtime state remain user data.

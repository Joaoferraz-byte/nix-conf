...
Ambxst is the sole user-facing shell, started exactly once by Niri through `spawn-at-startup`. The `ambxst-conf` flake owns the pinned runtime package and lifecycle contract; `shell-conf` supplies shell-independent application adapters through `support-core`. Noctalia is not imported into the active Home Manager composition.
...
`shell-conf` owns native GTK, Qt, Firefox, Zen Browser, WezTerm, Kitty, Starship and KDE contracts, plus formats not covered by those adapters, including Freesm Launcher, Heroic, Foliate, Xournal++, Vesktop and the Hydra export staging tree. Android Studio is installed without a repository-owned theme plugin or editor color scheme. Hydra's Appearance database remains application-owned and is not mutated by the adapter. The Livara Home Manager profile provides Nautilus and Spotify through Spicetify-Nix; optional Noctalia compatibility remains isolated from the active Ambxst path. Generated state is mutable runtime data and is never copied into the source tree.

Ambxst owns its pinned upstream modules and capture UI; `shell-conf` contributes only shell-independent adapters. Screen Toolkit remains the single owner of capture and recording in any compatibility shell, consuming the system-provided `gpu-screen-recorder`, `wl-screenrec` and Wayland/OCR/annotation tools from `features/niri.nix`. Plugin source is immutable; plugin settings and runtime state remain user data.

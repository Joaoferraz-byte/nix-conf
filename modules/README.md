...
`shell-conf` owns native GTK, Qt, Firefox, Zen Browser, WezTerm, Kitty, Starship and KDE contracts, plus formats not covered by those templates, including Freesm Launcher, Heroic, Foliate, Xournal++, Vesktop, IntelliJ/Android Studio, Telegram Desktop and Hydra source artifacts. The Livara Home Manager profile provides Nautilus, Spotify through Spicetify-Nix and local Books/Games/Musics directories. Generated state is mutable runtime data and is never copied into the source tree.

...
in
{
  ...
  home.activation.migrateLegacyHyprlandLua = lib.hm.dag.entryBefore [ "checkLinkTargets" ] ''
    ...
    if [ -f "$stale" ] && [ ! -L "$stale" ] && grep -Eq 'autogenerated[[:space:]]*=[[:space:]]*true|require[[:space:]]*\([[:space:]]*["'"']hyprland\.shell["'"']|(^|[[:space:]])(source|bind|env|input|general|decoration|windowrule|exec|exec-once)[[:space:]]*=|\$[A-Za-z_]' "$stale"; then

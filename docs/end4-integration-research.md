...
The upstream Hyprland startup file launches `qs -c $qsConfig`, fcitx5, gnome-keyring, a polkit agent, hypridle, DBus environment updates, EasyEffects, clipboard persistence, and cursor setup. It contains screenshot bindings using QuickShell screenshot UI with grim/hyprshot fallback, OCR using grim and slurp, color picking with hyprpicker, and fullscreen screenshots with grim. The upstream tree contains a recording script, but wf-recorder 0.6.0 does not build against the selected FFmpeg API. The local integration replaces that script with a declarative gpu-screen-recorder wrapper using the documented `-w`, `-region`, `-a`, and SIGINT interfaces. Screenshots remain on grim/grimblast/slurp/satty. GPU recording depends on the active Intel VAAPI stack and must be tested on the Latitude before being considered production-ready. The local integration keeps static assets read-only while creating writable custom and generated directories outside the Nix store.

...
## Lua module materialization

The Home Manager `extraLuaFiles` adapter now materializes upstream Lua modules with `builtins.readFile`. Passing the store path as textual content can produce a Lua file whose first line is an absolute path, leading to `unexpected symbol near '/'` when Hyprland executes `require`. The adapter keeps store paths for static `xdg.configFile` assets, but every Lua module supplied through `extraLuaFiles` is now read as source text.
